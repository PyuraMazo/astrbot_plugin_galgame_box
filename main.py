...
class GalgameBoxPlugin(Star):
    ...
    async def vn(self, event: AstrMessageEvent, keyword: str):
        """通过关键词查询作品"""
        yield await anext(self._single_image_command(event, CommandType.VN, keyword))

    @gal_box.command('角色', alias={'人物'})
    async def character(self, event: AstrMessageEvent, keyword: str):
        """通过关键词查询作品"""
        yield await anext(self._single_image_command(event, CommandType.CHARACTER, keyword))
    ...
    async def download(self, event: AstrMessageEvent, id: str):
        ...
        async for res in self._multi_feedback_command(event, CommandType.DOWNLOAD, id):
            yield res
