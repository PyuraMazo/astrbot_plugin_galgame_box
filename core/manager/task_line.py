...
from data.plugins.astrbot_plugin_galgame_box.core.utils.html_handler import (
    HTMLHandler,
    get_html_handler,
)
...
class TaskLine:
    ...
    async def _random_task(self, cmd_body: CommandBody):
        ...
        resp = (
            await self.touchgal_request.request_vn_by_search(
                details.title or details.third_info[1]
            )
        )[0]
        ...
        if keyword.isdigit():
            ...
            if total == 0:
                ...
                try:
                    ...
                    index = self.session_data_storage.pop(
                        event.get_group_id() + event.get_sender_id()
                    )
    ...
    async def _find_task(self, cmd_body: CommandBody):
        ...
        if not cmd_body.value:
            ...
            try:
                ...
                cmd_body.value = self.session_data_storage.pop(
                    event.get_group_id() + event.get_sender_id()
                )
            except TimeoutError:
                raise SessionTimeoutException(cmd_body)
    ...
    async def _recommend_task(self, cmd_body: CommandBody, **kwargs):
        ...
        if not kwargs.get("only_data", False):
            ...
            cache_body = SelectInfo(
                cmd_body=cmd_body, cache=data, current=1, total=total, tmpl=tmpl
            )
            url = await html_renderer.render_custom_template(
                cache_body.tmpl,
                    ...
                    if body.cache:
                        ...
                        if not body.cache:
                            if body.current < body.total:
                                new_resp = await anext(
                                    self._recommend_task(
                                        cmd_body,
                                        only_data=True,
                                        page=int(
                                            body.current / self.recommend_cache + 1
                                        ),
                                    )
                                )
                                body.cache = (
    ...
    async def _puzzle_task(self, cmd_body: CommandBody):
        ...
        if saved.record and now - saved.write_time <= 2 * 7 * 24 * 3600:
                ...
                for i in vndb_vns_list:
                    ...
                    for j in i.extlinks:
                        if j.label == "Steam":
                            ...
                            break
                saved.others_id = list(
                    set(saved.others_id) | (set(extra_id_list) - set(vn_id_list))
                )
            ...
            recent_dict = {
                ...
                if game.appid in saved.vns or game.appid in extra_vns_id_set
            }
            ...
            for m in extra_vns_id_set:
                saved.vns[m] = SteamVnsInfo(
                    name=recent_dict[m].name, play_time=0, vndb_img=extra_img_dict[m]
                )
        ...
        # 初始化和更新超过两周
            ...
            vndb_vns_list = await self.vndb_request.request_by_release(
                id_list, steam_owner.game_count
            )
            vndb_vns_dict = {
                int(j.id): i.vns[0].image.url
                for i in vndb_vns_list
                for j in i.extlinks
                if j.label == "Steam"
            }
